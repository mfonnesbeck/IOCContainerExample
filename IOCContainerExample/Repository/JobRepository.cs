using IOCContainerExample.Models;
using System;
using System.Collections.Generic;
using System.Linq;

namespace IOCContainerExample.Repository
{
    /// <summary>
    /// Job Repository, in normal application this would connect to and pull database information
    /// </summary>
    public class JobRepository : IJobRepository, IDisposable
    {
        //Data
        private List<JobItem> items = new List<JobItem>() { 
            new JobItem{jobID=1, jobCompany="Extend Health", jobContactPerson="Ryan Hauert", jobTitle="Senior Web Developer", jobUpdate=DateTime.Today},
            new JobItem{jobID=2, jobCompany="Microsoft", jobContactPerson="Steve Ballmer", jobTitle="Senior DotNet Engineer", jobUpdate=DateTime.Today},
            new JobItem{jobID=3, jobCompany="Google inc", jobContactPerson="Larry Page", jobTitle="Senior Web Services Developer", jobUpdate=DateTime.Today}
        };

        /// <summary>
        /// Gets a list of all Jobs
        /// </summary>
        /// <returns>A List of all Jobs</returns>
        public List<JobItem> GetAllJobs()
        {
            return items;
        }

        /// <summary>
        /// Returns a single sought-for Job
        /// </summary>
        /// <param name="id">Id for the Job</param>
        /// <returns>The found Job object</returns>
        public JobItem JobDetails(int id)
        {
            var item = items.Find(i => i.jobID == id);
            return item;
        }

        /// <summary>
        /// IDisposable method to properly dispose of the Job Repository object 
        /// </summary>
        public void Dispose()
        {
            items = null;
        }
    }
}